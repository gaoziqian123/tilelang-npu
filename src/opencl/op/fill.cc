/*!
 * \file tl/opencl/op/fill.cc
 * \brief OpenCL implementation for tl.fill lowering.
 */
...
bool MatchOpenCLFillTarget(Target target) {
  return target.defined() && target->kind.defined() &&
         target->kind->name == "opencl";
}

bool RegisterOpenCLFill() {
  RegisterFillImpl(FillImpl{
      "opencl.Fill",
      MatchOpenCLFillTarget,
      opencl::Fill::Lower,
  });
  return true;
}

const bool opencl_fill_registered = RegisterOpenCLFill();
