...
class HexagonEmitter(PyStmtExprVisitor):
    ...
    def _reset_func_state(self) -> None:
        ...
        self._loop_bounds: dict[str, tuple[str, str]] = {}
        self._fa_slice_from_abl = False
    ...
    def _lower_for(self, op: For, ctx: _Ctx, indent: int) -> list[str]:
        ...
        if var in self._loop_bounds:
            lo, hi = self._loop_bounds[var]
            return [self._ind(indent, f"for (int {var} = {lo}; {var} < {hi}; {var}++) {{")] + body + [self._ind(indent, "}")]
        return [self._ind(indent, f"for (int {var} = 0; {var} < {bound}; {var}++) {{")] + body + [self._ind(indent, "}")]
    ...
    def _record_params(self, func: PrimFunc) -> None:
        ...
        self._fa_slice_from_abl = _bool_arg(attrs.get("hexagon.fa_kv_slice_from_abl", False))
        self._prof_next_slot = 5
        ...
        raw_loop_bounds = attrs.get("hexagon.loop_bounds")
        if raw_loop_bounds is None:
            raw_loop_bounds = ""
        raw_loop_bounds_s = str(raw_loop_bounds.value if hasattr(raw_loop_bounds, "value") else raw_loop_bounds)
        self._loop_bounds = {}
        if raw_loop_bounds_s:
            for item in raw_loop_bounds_s.split(","):
                if not item:
                    continue
                name, lo, hi = item.split(":", 2)
                self._loop_bounds[name] = (lo, hi)
    ...
    def _render_c(self) -> str:
        ...
        if self._fa_slice_from_abl:
            # ``abl`` carries both the low-byte ablation flags and the FA
            # KV-head slice in bits [8:16].  Keep generated profiling / HMX
            # ablation checks byte-compatible with the pre-slice kernel by
            # masking the flag byte once in the entry prelude, then threading
            # that value through worker contexts and main-thread GEMM guards.
            body = body.replace(", abl, prof", ", abl_flags, prof")
            body = body.replace("!(abl &", "!(abl_flags &")
...
''')
    ...
    def _render_generic_c(self, m: int, n: int, k: int) -> str:
        ...
        slice_prelude = ""
        if self._fa_slice_from_abl:
            slice_prelude = """    int g_lo = (abl >> 8) & 0xf;
    int g_hi = (abl >> 12) & 0xf;
    if (g_hi == 0) g_hi = 4;
