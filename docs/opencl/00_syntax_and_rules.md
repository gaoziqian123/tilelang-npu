...
PASS,但性能只有 36ms,原因是无向量化、无 tile autotune、无 Adreno 私有矩阵扩展。
这不是性能上限声明,只是 v0.1 生成链可用性的锚点。
...
| L2 | GEMM NT 512³ fp16 | `T.copy` + `T.gemm` 高层写法,`opencl.fma` lowering | PASS, `max_rel=4.8e-4`,36ms |
...
1. **GEMM 向量化与 autotune**:引入 `half8` / `float4` 等向量化,搜索 tile 空间,
   对齐手写 Adreno OpenCL kernel 的 fp32 累加经验。
...
4. **清理 legacy 兼容层**:评估让 OpenCL 走 WebGPU `preserve_aliases=false` 路径,
   或自写 OpenCL codegen(照 `codegen_metal.cc`)以原生支持 `shared.dyn`、去掉 remap。
5. **长期突破口**:评估 Adreno 私有 `cl_qcom_ml_ops` 是否可接入矩阵路径;当前未做,
   不在 v0.1 承诺范围内。
