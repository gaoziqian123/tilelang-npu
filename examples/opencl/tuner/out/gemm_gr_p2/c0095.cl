...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A, __global half* restrict B, __global half* restrict C) { const int tl_gid0 = convert_int(get_group_id(0)); const int tl_gid1 = convert_int(get_group_id(1)); const int tl_lid0 = convert_int(get_local_id(0)); const int tl_wi_aff0 = ((tl_gid1 * 163840) + ((tl_lid0 >> 4) * 20480)); const int tl_wi_aff1 = ((tl_lid0 >> 4) * 327680); const int tl_wi_aff2 = (tl_gid0 * 327680); const int tl_wi_aff3 = (tl_lid0 * 20480);
  ...
  for (int pos4 = 0; pos4 < 640; ++pos4) { const int tl_loop_aff0 = (tl_wi_aff0 + (pos4 * 4)); const int tl_loop_aff1 = (pos4 * 4);
    float4 av_1[8];
    float4 bg_1[8];
    ...
    bg_1[0] = (convert_float4(vload4(0, B + (((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) - tl_wi_aff1))));
    bg_1[1] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 2560) - tl_wi_aff1))));
    bg_1[2] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 5120) - tl_wi_aff1))));
    bg_1[3] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 7680) - tl_wi_aff1))));
    bg_1[4] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 10240) - tl_wi_aff1))));
    bg_1[5] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 12800) - tl_wi_aff1))));
    bg_1[6] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 15360) - tl_wi_aff1))));
    bg_1[7] = (convert_float4(vload4(0, B + ((((tl_wi_aff2 + tl_wi_aff3) + tl_loop_aff1) + 17920) - tl_wi_aff1))));
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    vstore8((*(half8*)(C_local_cast + 0)), 0, C + (((tl_wi_aff0 + (i * 2560)) + (tl_gid0 * 128)) + ((tl_lid0 & 15) * 8)));
  }
}
