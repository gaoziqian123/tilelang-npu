...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A, __global half* restrict B, __global half* restrict C) { const int tl_gid0 = convert_int(get_group_id(0)); const int tl_gid1 = convert_int(get_group_id(1)); const int tl_lid0 = convert_int(get_local_id(0)); const int tl_wi_aff0 = ((tl_gid1 * 163840) + ((tl_lid0 >> 4) * 20480)); const int tl_wi_aff1 = ((tl_gid1 * 589824) + ((tl_lid0 >> 4) * 73728)); const int tl_wi_aff2 = ((tl_wi_aff0 + (tl_gid0 * 128)) + ((tl_lid0 & 15) * 8)); const int tl_wi_aff3 = ((tl_lid0 >> 4) * 128); const int tl_wi_aff4 = (tl_gid0 * 128); const int tl_wi_aff5 = (tl_lid0 * 8);
  ...
  for (int pos4 = 0; pos4 < 2304; ++pos4) { const int tl_loop_aff0 = (tl_wi_aff1 + (pos4 * 4)); const int tl_loop_aff1 = (((pos4 * 10240) + tl_wi_aff4) + tl_wi_aff5);
    float4 av_1[8];
    float8 bv_1[4];
    av_1[0] = (convert_float4(vload4(0, A + tl_loop_aff0)));
    av_1[1] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 9216))));
    av_1[2] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 18432))));
    av_1[3] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 27648))));
    av_1[4] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 36864))));
    av_1[5] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 46080))));
    av_1[6] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 55296))));
    av_1[7] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 64512))));
    bv_1[0] = (convert_float8(vload8(0, B + (tl_loop_aff1 - tl_wi_aff3))));
    bv_1[1] = (convert_float8(vload8(0, B + ((tl_loop_aff1 + 2560) - tl_wi_aff3))));
    bv_1[2] = (convert_float8(vload8(0, B + ((tl_loop_aff1 + 5120) - tl_wi_aff3))));
    bv_1[3] = (convert_float8(vload8(0, B + ((tl_loop_aff1 + 7680) - tl_wi_aff3))));
  ...
  vstore8(convert_half8(acc_1_1), 0, C + (tl_wi_aff2 + 2560));

  vstore8(convert_half8(acc_1_2), 0, C + (tl_wi_aff2 + 5120));

  vstore8(convert_half8(acc_1_3), 0, C + (tl_wi_aff2 + 7680));

  vstore8(convert_half8(acc_1_4), 0, C + (tl_wi_aff2 + 10240));

  vstore8(convert_half8(acc_1_5), 0, C + (tl_wi_aff2 + 12800));

  vstore8(convert_half8(acc_1_6), 0, C + (tl_wi_aff2 + 15360));

  vstore8(convert_half8(acc_1_7), 0, C + (tl_wi_aff2 + 17920));
