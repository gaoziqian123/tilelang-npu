...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A_1, __global half* restrict B_1, __global half* restrict C_1) {
  ...
  for (int kb = 0; kb < 2560; kb += 16) {
    for (int v = (convert_int(get_local_id(0))); v < 256; v += 64) {
      int r = (v >> 2);
      int c4 = (v - ((v >> 2) * 4));
      half4 aval = vload4(0, ((__global half *)A_1 + (((((convert_int(get_group_id(1))) * 163840) + ((v >> 2) * 2544)) + (v * 4)) + kb)));
      As[((v * 256) - ((v >> 2) * 1023))] = (aval).s0;
      As[(((v * 256) + 64) - ((v >> 2) * 1023))] = (aval).s1;
      As[(((v * 256) + 128) - ((v >> 2) * 1023))] = (aval).s2;
      As[(((v * 256) + 192) - ((v >> 2) * 1023))] = (aval).s3;
    }
    for (int v_1 = (convert_int(get_local_id(0))); v_1 < 128; v_1 += 64) {
      int k = (v_1 >> 3);
      int c8 = (v_1 - ((v_1 >> 3) * 8));
