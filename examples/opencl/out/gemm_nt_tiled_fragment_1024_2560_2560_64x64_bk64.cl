...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A_1, __global half* restrict B_1, __global half* restrict C_1) {
  ...
  for (int kb = 0; kb < 2560; kb += 64) {
    for (int v = (convert_int(get_local_id(0))); v < 1024; v += 64) {
      int r = (v >> 4);
      int c4 = (v - ((v >> 4) * 16));
    ...
    for (int v_1 = (convert_int(get_local_id(0))); v_1 < 1024; v_1 += 64) {
      int c = (v_1 >> 4);
      int k4 = (v_1 - ((v_1 >> 4) * 16));
      half4 bval = vload4(0, ((__global half *)B_1 + (((((convert_int(get_group_id(0))) * 163840) + ((v_1 >> 4) * 2496)) + (v_1 * 4)) + kb)));
      Bs[((v_1 * 256) - ((v_1 >> 4) * 4095))] = (bval).s0;
      Bs[(((v_1 * 256) + 64) - ((v_1 >> 4) * 4095))] = (bval).s1;
      Bs[(((v_1 * 256) + 128) - ((v_1 >> 4) * 4095))] = (bval).s2;
      Bs[(((v_1 * 256) + 192) - ((v_1 >> 4) * 4095))] = (bval).s3;
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    for (int kk = 0; kk < 64; ++kk) {
      float8 a8 = (convert_float8(vload8(0, As + ((kk * 64) + (((convert_int(get_local_id(0))) >> 3) * 8)))));
