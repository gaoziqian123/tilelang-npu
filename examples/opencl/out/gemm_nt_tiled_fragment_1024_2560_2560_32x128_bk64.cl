...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A_1, __global half* restrict B_1, __global half* restrict C_1) {
  ...
  for (int kb = 0; kb < 2560; kb += 64) {
    for (int v = (convert_int(get_local_id(0))); v < 512; v += 64) {
      int r = (v >> 4);
      int c4 = (v - ((v >> 4) * 16));
      half4 aval = vload4(0, ((__global half *)A_1 + (((((convert_int(get_group_id(1))) * 81920) + ((v >> 4) * 2496)) + (v * 4)) + kb)));
      As[((v * 128) - ((v >> 4) * 2047))] = (aval).s0;
      As[(((v * 128) + 32) - ((v >> 4) * 2047))] = (aval).s1;
      As[(((v * 128) + 64) - ((v >> 4) * 2047))] = (aval).s2;
      As[(((v * 128) + 96) - ((v >> 4) * 2047))] = (aval).s3;
    }
    for (int v_1 = (convert_int(get_local_id(0))); v_1 < 2048; v_1 += 64) {
      int c = (v_1 >> 4);
      int k4 = (v_1 - ((v_1 >> 4) * 16));
