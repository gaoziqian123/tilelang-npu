...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A_1, __global half* restrict B_1, __global half* restrict C_1) {
  ...
  for (int kb = 0; kb < 2560; kb += 32) {
    for (int v = (convert_int(get_local_id(0))); v < 512; v += 128) {
      int r = (v >> 3);
      int c4 = (v - ((v >> 3) * 8));
      half4 aval = vload4(0, ((__global half *)A_1 + (((((convert_int(get_group_id(1))) * 163840) + ((v >> 3) * 2528)) + (v * 4)) + kb)));
      As[((v * 256) - ((v >> 3) * 2047))] = (aval).s0;
      As[(((v * 256) + 64) - ((v >> 3) * 2047))] = (aval).s1;
      As[(((v * 256) + 128) - ((v >> 3) * 2047))] = (aval).s2;
      As[(((v * 256) + 192) - ((v >> 3) * 2047))] = (aval).s3;
    }
    for (int v_1 = (convert_int(get_local_id(0))); v_1 < 1024; v_1 += 128) {
      int c = (v_1 >> 3);
      int k4 = (v_1 - ((v_1 >> 3) * 8));
      half4 bval = vload4(0, ((__global half *)B_1 + (((((convert_int(get_group_id(0))) * 327680) + ((v_1 >> 3) * 2528)) + (v_1 * 4)) + kb)));
      Bs[((v_1 * 512) - ((v_1 >> 3) * 4095))] = (bval).s0;
      Bs[(((v_1 * 512) + 128) - ((v_1 >> 3) * 4095))] = (bval).s1;
      Bs[(((v_1 * 512) + 256) - ((v_1 >> 3) * 4095))] = (bval).s2;
      Bs[(((v_1 * 512) + 384) - ((v_1 >> 3) * 4095))] = (bval).s3;
    }
