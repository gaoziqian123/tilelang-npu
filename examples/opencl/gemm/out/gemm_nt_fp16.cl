...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A, __global half* restrict B, __global half* restrict C) { const int tl_gid0 = convert_int(get_group_id(0)); const int tl_gid1 = convert_int(get_group_id(1)); const int tl_lid0 = convert_int(get_local_id(0)); const int tl_wi_aff0 = ((tl_gid1 * 81920) + ((tl_lid0 >> 4) * 20480)); const int tl_wi_aff1 = ((tl_wi_aff0 + (tl_gid0 * 128)) + ((tl_lid0 & 15) * 8)); const int tl_wi_aff2 = ((tl_lid0 >> 4) * 128); const int tl_wi_aff3 = (tl_gid0 * 128); const int tl_wi_aff4 = (tl_lid0 * 8); __global half* tl_C_base = C + tl_wi_aff1;
  ...
  for (int pos4 = 0; pos4 < 640; ++pos4) { const int tl_loop_aff0 = (tl_wi_aff0 + (pos4 * 4)); const int tl_loop_aff1 = (((pos4 * 10240) + tl_wi_aff3) + tl_wi_aff4);
    half4 av_1[8];
    half8 bv_1[4];
    av_1[0] = vload4(0, A + tl_loop_aff0);
    av_1[1] = vload4(0, A + (tl_loop_aff0 + 2560));
    av_1[2] = vload4(0, A + (tl_loop_aff0 + 5120));
    av_1[3] = vload4(0, A + (tl_loop_aff0 + 7680));
    av_1[4] = vload4(0, A + (tl_loop_aff0 + 10240));
    av_1[5] = vload4(0, A + (tl_loop_aff0 + 12800));
    av_1[6] = vload4(0, A + (tl_loop_aff0 + 15360));
    av_1[7] = vload4(0, A + (tl_loop_aff0 + 17920));
    bv_1[0] = vload8(0, B + (tl_loop_aff1 - tl_wi_aff2));
    bv_1[1] = vload8(0, B + ((tl_loop_aff1 + 2560) - tl_wi_aff2));
    bv_1[2] = vload8(0, B + ((tl_loop_aff1 + 5120) - tl_wi_aff2));
    bv_1[3] = vload8(0, B + ((tl_loop_aff1 + 7680) - tl_wi_aff2));
  ...
  vstore8((*(half8*)(C_frag_1 + 8)), 0, tl_C_base + ( + 2560));
  vstore8((*(half8*)(C_frag_1 + 16)), 0, tl_C_base + ( + 5120));
  vstore8((*(half8*)(C_frag_1 + 24)), 0, tl_C_base + ( + 7680));
  vstore8((*(half8*)(C_frag_1 + 32)), 0, tl_C_base + ( + 10240));
  vstore8((*(half8*)(C_frag_1 + 40)), 0, tl_C_base + ( + 12800));
  vstore8((*(half8*)(C_frag_1 + 48)), 0, tl_C_base + ( + 15360));
  vstore8((*(half8*)(C_frag_1 + 56)), 0, tl_C_base + ( + 17920));
}
