...
__kernel void gemm_nt_kernel_kernel(__global half* restrict A, __global half* restrict B, __global half* restrict C) { const int tl_gid0 = convert_int(get_group_id(0)); const int tl_gid1 = convert_int(get_group_id(1)); const int tl_lid0 = convert_int(get_local_id(0)); const int tl_wi_aff0 = ((tl_gid1 * 131072) + ((tl_lid0 >> 5) * 32768)); const int tl_wi_aff1 = ((tl_gid1 * 32768) + ((tl_lid0 >> 5) * 8192)); const int tl_wi_aff2 = ((tl_wi_aff1 + (tl_gid0 * 256)) + ((tl_lid0 & 31) * 8)); const int tl_wi_aff3 = ((tl_lid0 >> 5) * 256); const int tl_wi_aff4 = (tl_gid0 * 256); const int tl_wi_aff5 = (tl_lid0 * 8);
  ...
  for (int pos4 = 0; pos4 < 1024; ++pos4) { const int tl_loop_aff0 = (tl_wi_aff0 + (pos4 * 4)); const int tl_loop_aff1 = (((pos4 * 4096) + tl_wi_aff4) + tl_wi_aff5);
    float4 av_1[8];
    float8 bv_1[4];
    av_1[0] = (convert_float4(vload4(0, A + tl_loop_aff0)));
    av_1[1] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 4096))));
    av_1[2] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 8192))));
    av_1[3] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 12288))));
    av_1[4] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 16384))));
    av_1[5] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 20480))));
    av_1[6] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 24576))));
    av_1[7] = (convert_float4(vload4(0, A + (tl_loop_aff0 + 28672))));
    bv_1[0] = (convert_float8(vload8(0, B + (tl_loop_aff1 - tl_wi_aff3))));
    bv_1[1] = (convert_float8(vload8(0, B + ((tl_loop_aff1 + 1024) - tl_wi_aff3))));
    bv_1[2] = (convert_float8(vload8(0, B + ((tl_loop_aff1 + 2048) - tl_wi_aff3))));
    bv_1[3] = (convert_float8(vload8(0, B + ((tl_loop_aff1 + 3072) - tl_wi_aff3))));
  ...
  vstore8(convert_half8(acc_1_1), 0, C + (tl_wi_aff2 + 1024));

  vstore8(convert_half8(acc_1_2), 0, C + (tl_wi_aff2 + 2048));

  vstore8(convert_half8(acc_1_3), 0, C + (tl_wi_aff2 + 3072));

  vstore8(convert_half8(acc_1_4), 0, C + (tl_wi_aff2 + 4096));

  vstore8(convert_half8(acc_1_5), 0, C + (tl_wi_aff2 + 5120));

  vstore8(convert_half8(acc_1_6), 0, C + (tl_wi_aff2 + 6144));

  vstore8(convert_half8(acc_1_7), 0, C + (tl_wi_aff2 + 7168));
