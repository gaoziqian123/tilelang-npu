...
__local half UW[4096];
  ...
  if (tl_lid0 < 32) {
    ...
    gc[tl_lid0] = s_1;
    egc[tl_lid0] = e_1;
    EgcBuf[((tl_wi_aff4 + tl_wi_aff7) + tl_lid0)] = e_1;
  }
  barrier(CLK_LOCAL_MEM_FENCE);
  ...
  barrier(CLK_LOCAL_MEM_FENCE);
  for (int i = 0; i < 32; ++i) {
    float au_1 = (bf[i] * (convert_float(V[(((tl_wi_aff2 + tl_wi_aff5) + (i * 128)) + tl_lid0)])));
    for (int m = 0; m < 32; ++m) {
      if (m < i) {
        au_1 = (au_1 - (L[((i * 32) + m)] * (convert_float(UW[((m * 128) + tl_lid0)]))));
      }
    }
    UW[((i * 128) + tl_lid0)] = (convert_half(au_1));
  }
  ...
  for (int i_2 = 0; i_2 < 32; ++i_2) {
    float aw_1[1];
    aw_1[0] = ((bf[i_2] * (convert_float(K[(((((tl_gid1 & 15) * 131072) + tl_wi_aff5) + (i_2 * 128)) + tl_lid0)]))) * egc[i_2]);
  ...
  for (int i_3 = 0; i_3 < 32; ++i_3) { const int tl_loop_aff0 = (i_3 * 128);
    ...
    KDbuf[(((tl_wi_aff2 + tl_wi_aff5) + tl_loop_aff0) + tl_lid0)] = (convert_half(((convert_float(K[(((((tl_gid1 & 15) * 131072) + tl_wi_aff5) + tl_loop_aff0) + tl_lid0)])) * native_exp((gc[31] - gc[i_3])))));
  }
}
