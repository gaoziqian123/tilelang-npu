...
__local half UW[4096];
  if (tl_lid0 < 32) {
    gf[tl_lid0] = G[((tl_wi_aff4 + tl_wi_aff7) + tl_lid0)];
    bf[tl_lid0] = B[((tl_wi_aff4 + tl_wi_aff7) + tl_lid0)];
  }
  ...
  if (tl_lid0 < 32) {
    ...
    EgcBuf[((tl_wi_aff4 + tl_wi_aff7) + tl_lid0)] = native_exp(s_1);
  }
  barrier(CLK_LOCAL_MEM_FENCE);
  ...
  for (int i = 0; i < 32; ++i) {
    float au_1[1];
    au_1[0] = (bf[i] * (convert_float(V[(((tl_wi_aff2 + tl_wi_aff6) + (i * 128)) + tl_lid0)])));
    for (int m = 0; m < 32; ++m) {
      if (m < i) {
  ...
  for (int i_1 = 0; i_1 < 32; ++i_1) { const int tl_loop_aff0 = (i_1 * 128);
    Ubuf[(((tl_wi_aff2 + tl_wi_aff6) + tl_loop_aff0) + tl_lid0)] = UW[(tl_loop_aff0 + tl_lid0)];
  }
  barrier(CLK_LOCAL_MEM_FENCE);
  for (int i_2 = 0; i_2 < 32; ++i_2) {
    float aw_1[1];
    aw_1[0] = ((bf[i_2] * (convert_float(K[(((((tl_gid1 & 15) * 131072) + tl_wi_aff6) + (i_2 * 128)) + tl_lid0)]))) * egc[i_2]);
  ...
  for (int i_3 = 0; i_3 < 32; ++i_3) { const int tl_loop_aff0 = (i_3 * 128);
    Wbuf[(((tl_wi_aff2 + tl_wi_aff6) + tl_loop_aff0) + tl_lid0)] = UW[(tl_loop_aff0 + tl_lid0)];
    KDbuf[(((tl_wi_aff2 + tl_wi_aff6) + tl_loop_aff0) + tl_lid0)] = (convert_half(((convert_float(K[(((((tl_gid1 & 15) * 131072) + tl_wi_aff6) + tl_loop_aff0) + tl_lid0)])) * native_exp((gc[31] - gc[i_3])))));
  }
}
