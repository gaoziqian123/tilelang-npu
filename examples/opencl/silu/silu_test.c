// silu_test.c - host validation for the TileLang-emitted silu OpenCL kernel.
//
// Verifies O[i] = A[i] / (1 + exp(-A[i])) against an fp64 reference with the
// rms-scaled relative error gate |got-ref|/(|ref|+0.02*rms).
//
...
//     -I/root/project/backend/gpu/OpenCL-Headers silu_test.c -ldl -lm -o silu_test
// Run (device):
//   LD_LIBRARY_PATH=.:/system/lib64:/vendor/lib64 ./silu_test out/silu.cl silu_kernel_kernel
//
// Env: TL_ROWS (8192) TL_COLS (960) TL_THREADS (256)  (nelem = rows*cols)
...
int main(int argc, char **argv) {
    const char *cl_path = argc > 1 ? argv[1] : "out/silu.cl";
    const char *kname = argc > 2 ? argv[2] : "silu_kernel_kernel";
    const int R = env_i("TL_ROWS", 8192), C = env_i("TL_COLS", 960);
    ...
    if (!A || !O || !ref) { fprintf(stderr, "silu alloc failed\n"); exit(2); }
    for (size_t i = 0; i < n; ++i) {
        A[i] = rand_f32(&seed);
        double x = (double)A[i];
        ref[i] = x / (1.0 + exp(-x));
    ...
    const size_t local[1] = {(size_t)THREADS}, global[1] = {n};
    double t0 = now_s();
    ...
    printf("SILU n=%zu TH=%d ran %.3f ms\n", n, THREADS, (t1 - t0) * 1e3);
    int rc = check_f32("SILU", O, ref, n);
